...
namespace max7219_matrix {
    ...
    const _NOenum rotation_direction {
    //% block="none"
    none = 0,
    //% block="clockwise"
    clockwise = 1,
...
namespace max7219_matrix {
    ...
    //MAX7219的寄存器
    const _NOOP=0//没有任何操作，不改变当前状态
	const _DIGIT=[1,2,3,4,5,6,7,8]//（数字）LED灯的列数
	const _DECODEMODE=9//译码控制寄存器，1表示on，0表示off，用于max7219上的7段显示，这里没有使用
	const _INTENSITY=10//强度，LED灯的亮度，0-15
	const _SCANLIMIT=11//扫描位数，限制寄存器
	const _SHUTDOWN=12//turn on(1) or off(0)
	const _DISPLAYTEST=15  //

    let _pinCS = DigitalPin.P16 // LOAD pin, 0代表准备接收数字，1表示使命令生效
    let _matrixNum = 1 //链中连接的矩阵的数目
    let _displayArray: number[] = [] //显示数组，给再所有的矩阵上显示做准备
    let _rotation = 0 // 旋转矩阵显示为4-in-1 模块
    let _reversed = false //4-in-1 modules的反转矩阵的显示顺序
